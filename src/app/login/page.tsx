...
export default function LoginPage() {
  ...
  React.useEffect(() => {
    // This effect now only runs if a user is already logged in when the page loads.
    if (!loading && user) {
      router.push('/');
    }
  }, [user, loading, router]);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // On success, we manually redirect. The `onAuthStateChanged` listener will have
      // already updated the user state, but we control the navigation here.
      router.push('/');
    } catch (error: any) {
      if (error.code === 'auth/popup-closed-by-user') {
        // This is a user action (closing the popup), not an error.
  ...
  // Display a loader while the initial auth state is being determined.
  if (loading) {
    return (
  ...
  // If the user is logged in (e.g., after a successful sign-in), this loader will show during the redirect.
  if (user) {
    return (
        <div className="flex min-h-screen flex-col items-center justify-center">
            <Loader className="h-12 w-12 animate-spin text-primary" />
